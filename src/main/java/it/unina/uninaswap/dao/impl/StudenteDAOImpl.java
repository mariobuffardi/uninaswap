...
public class StudenteDAOImpl implements StudenteDAO {
    ...
    @Override
    public boolean existsByMatricola(String matricola) throws Exception {
        String sql = "SELECT 1 FROM studente WHERE matricola = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, matricola);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    @Override
    public void insert(Studente s) throws Exception {
        try (Connection conn = DBConnection.getConnection()) {
            insert(s, conn);
        }
    }
    ...
    @Override
    public void insert(Studente s, Connection conn) throws Exception {
        String sql = "INSERT INTO studente (matricola, email, nome, cognome, password, sesso, preferisce_spedizione, preferisce_incontro_in_uni) " +
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setString(6, s.getSesso().toString());
